...
namespace YooAsset.Editor
{
    ...
    public class AssetBundleCollector
    {
        ...
        private CollectAssetInfo CreateCollectAssetInfo(CollectCommand command, AssetBundleCollectorGroup group, AssetInfo assetInfo)
        {
            ...
            if (UserData.Contains("TopTag"))
            {
                string subPath = assetInfo.AssetPath.Substring(CollectPath.Length + 1);
                string topTab = System.IO.Path.GetFileName(CollectPath);
                int subIndex = subPath.IndexOf("/");
                if (subIndex != -1)
                {
                    topTab += "/" + subPath.Substring(0, subIndex);
                }
                assetTags.Add(topTab);
            }
